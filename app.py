        ...
        try:
            ...
            if st.button("🔢 CALCULAR E GERAR SHIPPERS", use_container_width=True):
                ...
                if valores_vazios:
                    ...
                    with ZipFile(zip_buffer, "w") as zip_file:
                        for sigla in lista_siglas:
                            ...
                            if p_original is not None and p_original > 0:
                                ...
                                # SEU CÁLCULO ORIGINAL RESTAURADO:
                                f_sacas = float(qtd_sacas_escolhida)
                                
                                # 1. Coluna G: Peso Corrigido
                                g_peso_corrigido = (f_sacas * 3.0) + p_original
                                
                                # 2. Coluna I: Fibreboard Boxes
                                fracao_fib = Decimal(str(q_volumes)) / Decimal(str(f_sacas))
                                i_fibreboard = int(
                                    fracao_fib.quantize(
                                        Decimal('1'), 
                                ...
                                base_j_float = g_peso_corrigido / f_sacas / float(i_fibreboard)
                                ...
                                for acrescimo in range(2000): 
                                    ...
                                    # M = (Sacas * J * I) - G
                                    l_total_destino = (
                                        j_teste * Decimal(str(i_fibreboard)) * Decimal(str(f_sacas))
                                    )
                                    m_conferencia = (
                                        l_total_destino - Decimal(f"{g_peso_corrigido:.4f}")
                                    )
                                ...
                                j7_kg_g = perfeito_j
                                k7_total_saca_final = j7_kg_g * Decimal(str(i_fibreboard))

                                # Formatação e geração de variáveis do Word
                                ...
                                # RichText isolado para Arial Black tamanho 8 (size=16)
                    ...
                    if emitidos:
                        zip_buffer.seek(0)
                        st.success("✅ Sucesso! Shippers geradas com seu cálculo original.")
                        st.download_button(
