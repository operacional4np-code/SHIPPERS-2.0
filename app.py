...
# MAPA DE TRADUÇÃO DAS CIDADES para busca na planilha de coleta (Agora aceita espaço normal)
MAPA_DESTINOS = {
    "CGR": "CAMPO GRANDE", 
    "CGB": "CUIABA", 
    "CWB": "CURITIBA", 
...
def extrair_dados_coleta(df_raw, termo_busca):
    ...
    for idx in range(linha_cabecalho + 1, len(df_raw)):
        ...
        # Garante a comparação separando as palavras chaves
        palavras_busca = set(termo_busca.replace("-", " ").split())
        palavras_linha = set(val_destino.replace("-", " ").split())
        
        if keywords_match := (
            palavras_busca.issubset(palavras_linha)
            or palavras_linha.issubset(palavras_busca)
            or termo_busca in val_destino
        ):
            try:
...
sacas_manuais = {}
if siglas_input:
    # Correção crucial: Agora divide EXCLUSIVAMENTE por vírgula, permitindo espaços no nome
    ...
    todas_sacas_preenchidas = all(saca is not None for saca in sacas
